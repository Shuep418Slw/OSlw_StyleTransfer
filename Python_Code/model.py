...
#and I modify some code for running this on raspberry zeros W
#because the original model used more ram space
#you can download original code and replace its model.py with this
 

import tensorflow as tf


def conv2d(x, input_filters, output_filters, kernel, strides, mode='CONSTANT'):
...
def instance_norm2(x,input_filters):
    epsilon = 1e-9
    mean, var = tf.nn.moments(x, [1, 2], keep_dims=True)
    gamma=tf.Variable(tf.ones((1,1,1,input_filters),dtype=tf.float32), name='IN_gamma')
    beta=tf.Variable(tf.zeros((1,1,1,input_filters),dtype=tf.float32), name='IN_beta')
...
def net(image, training):
    ...
    with tf.variable_scope('conv1'):
        conv1 = relu(instance_norm2(conv2d(image, 3, 32, 9, 1),32))
    with tf.variable_scope('conv2'):
        conv2 = relu(instance_norm2(conv2d(conv1, 32, 64, 3, 2),64))
    with tf.variable_scope('conv3'):
        conv3 = relu(instance_norm2(conv2d(conv2, 64, 128, 3, 2),128))
    with tf.variable_scope('res1'):
        res1 = residual(conv3, 128, 3, 1)
    with tf.variable_scope('res2'):
        res2 = residual(res1, 128, 3, 1)
    ...
    with tf.variable_scope('deconv1'):
        # deconv1 = relu(instance_norm(conv2d_transpose(res5, 128, 64, 3, 2)))
        deconv1 = relu(instance_norm2(resize_conv2d(res5, 128, 64, 3, 1, training),64))
    with tf.variable_scope('deconv2'):
        # deconv2 = relu(instance_norm(conv2d_transpose(deconv1, 64, 32, 3, 2)))
        deconv2 = relu(instance_norm2(resize_conv2d(deconv1, 64, 32, 3, 1, training),32))
    with tf.variable_scope('deconv3'):
        # deconv_test = relu(instance_norm(conv2d(deconv2, 32, 32, 2, 1)))
        deconv3 = tf.nn.tanh(instance_norm2(conv2d(deconv2, 32, 3, 9, 1),3))
